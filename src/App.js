...
useEffect(() => {
  ...
  let paginate = (index) => {
    let start = parseInt(perPage) * index;
  ...
  <ul class="pagination">
    <li class="page-item">
      {
        currentPage !== 0 ? <button class="page-link" onClick={prev}>
          Previous</button> : null
      }
      </li>
    {
      [...Array(data.length / perPage)].map((item,index) => {
        return (
        <li class="page-item">
          <button className={`page-link ${currentPage === index ? "active": ""}`} onClick={() => paginate(index)}>
          {index + 1}             
          </button></li>
        );
      })}
       <li class="page-item">
        {
          currentPage !== (data.length / perPage) -1 ?  <button disabled={currentPage === (data.length / 5) -1} onClick={next} class="btn">Next</button> : null
        }
        </li>
  </ul>
  <select onChange={(e) => {
